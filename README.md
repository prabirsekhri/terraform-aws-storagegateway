...
This release adds Amazon Linux 2023 support and requires updated provider versions:

- Terraform >= 1.5.7, AWS Provider >= 5.0.0, AWSCC >= 1.0.0, vSphere >= 2.2.0

Key changes:

- EC2 and VMware modules now deploy AL2023-based Storage Gateway images
- Optional string variables (`domain_name`, `domain_password`, etc.) now default to `null` instead of `""`
- EC2 instances ignore AMI changes by default to prevent unexpected replacements

Run `terraform init -upgrade` after updating.
...
- `security_group_id` in the ec2-sgw module changed from `string` to `list(string)` to support multiple security groups. Update existing usage: `security_group_id = "sg-xxx"` → `security_group_id = ["sg-xxx"]`
- New migration example (`s3-filegateway-migration-ec2`) for automating AL2 to AL2023 gateway migration
...
The module requires a Gateway type to be declared. The default is configured to FILE\_S3 as an example. For more details regarding the Storage Gateway types and their respective arguments can be found in the [Terraform AWS Storage Gateway documentation](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/storagegateway_gateway).

## Usage with VMware S3 File Gateway module

Link to the S3 SMB Storage Gateway example for VMware: [s3-smb-filegateway-vmware](examples/s3-smb-filegateway-vmware)

### Prerequisites

The VMware module requires the vSphere provider to be setup with a service account user name and password that has the necessary permissions in vCenter to create a VM. This is found in the [settings.tf](examples/s3-smb-filegateway-vmware/settings.tf) file.
...
### Storage Gateway Module for EC2

Once the EC2 Gateway is deployed, pass the `activation_ip` output to the aws-sgw module for gateway activation.
...
module "sgw" {
  ...
  gateway_ip_address = module.ec2_sgw.activation_ip
  join_smb_domain    = false
  gateway_type       = "FILE_S3"
}
```

### Private VPC Activation for EC2 Gateway

For private deployments without public internet access, set `create_eip = false` in the ec2-sgw module. This uses the private IP for activation instead of creating an Elastic IP.
...
module "ec2_sgw" {
  ...
  subnet_id         = "subnet-abcdef123456"
  name              = "my-storage-gateway"
  create_eip        = false  # Use private IP for activation
}
...
module "sgw" {
  ...
  source             = "aws-ia/storagegateway/aws//modules/aws-sgw"
  gateway_name       = "my-storage-gateway"
  gateway_ip_address = module.ec2_sgw.activation_ip  # Returns private IP when create_eip=false
  join_smb_domain    = false
  gateway_type       = "FILE_S3"
}
```

Private activation requires network connectivity from Terraform to the gateway's private IP on port 80. See [Activating a gateway in a virtual private cloud](https://docs.aws.amazon.com/filegateway/latest/files3/gateway-private-link.html) for details.
...
| <a name="input_domain_username"></a> [domain\_username](#input\_domain\_username) | The user name for the service account on your self-managed AD domain that SGW use to join to your AD domain | `string` | `null` | no |
...
| <a name="input_organizational_unit"></a> [organizational\_unit](#input\_organizational\_unit) | The organizational unit (OU) is a container in an Active Directory that can hold users, groups, computers, and other OUs and this parameter specifies the OU that the gateway will join within the AD domain. | `string` | `null` | no |
